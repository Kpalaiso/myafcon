...
if (isset($templates[$args['type']][$args['template_'.$args['type']]])) {
    ...
    if( fcunited_exists_sportspress() ) {
        if ($args['taxonomy'] == 'sp_league' || 'sp_season' && fcunited_exists_sportspress()) {
            if ($args['post_type'] == 'sp_staff') {
                $staff = new SP_Staff(get_the_id());
                $roles = $staff->roles();
                if (is_array($roles) && !empty($roles)) {
                    $show_meta = false;
                    $info_out = '<span class="meta-role-wrap">';
                    foreach ($roles as $k) {
                        $info_out .= '<span class="item-role">' . $k->name . '</span>';
                    }
                    $info_out .= '</span>';
                }
                ...
                if(is_array($number) && !empty($number)){
                    $info_player_number = '<span class="player-number">' . $number[0] . '</span>';
                }
																...
																'thumb_size' => apply_filters(
																					'trx_addons_filter_thumb_size',
																						...
																						!in_array($args['image_ratio'], array('', 'none'))
																							? 'big'
																							: ((int)$args['columns'] > 4 ? 'height' : 'alternate')
																					),
																					'blogger-'.$args['type']
																				),
                                                                'post_info' => $info_player_number,
																), 'blogger-'.$args['type'])
										);
