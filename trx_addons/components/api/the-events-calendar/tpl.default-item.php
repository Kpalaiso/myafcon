...
<div class="sc_events_item">
	<?php
    $dat = '<div class="sc_events_item_date">'
	...
	<div class="sc_events_item_info">
		<div class="sc_events_item_header">
			...
			<div class="sc_events_item_meta">
				<span class="sc_events_item_meta_item sc_events_item_meta_date">
                    <span class="label_event"><?php esc_html_e('Time: ', 'fcunited'); ?></span><?php
					if ($dt == $dt2) {
						if ( $tm == null ) {
							trx_addons_show_layout(date_i18n(get_option('date_format'), strtotime($dt)), '<span class="sc_events_item_meta_date_start">', '</span>');
						} else {
							trx_addons_show_layout($tm, '<span class="sc_events_item_meta_time_start">', '</span>');
						}
					} else {
						if ( $tm == null ) {
							trx_addons_show_layout(date_i18n(get_option('date_format'), strtotime($dt)), '<span class="sc_events_item_meta_date_start">', '</span>');
                ...
                <?php
                    ...
                    if (!empty($cost)) {
                    ?>
                    <span class="sc_events_item_meta_item sc_events_item_price"><span class="label_event"><?php echo esc_html__('Cost: ', 'fcunited').'</span>'. esc_html(!empty($cost) ? $cost : esc_html__('free', 'fcunited')); ?></span>
